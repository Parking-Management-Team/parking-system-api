...
* **Single Responsibility Principle (SRP - Đơn nhiệm):**
  ...
  * Các tác vụ định kỳ chạy ngầm (Dọn dẹp Booking quá hạn, Cảnh báo quá hạn đỗ xe) đã được chuyển thành các Hosted Services chuyên biệt (`ExpiredBookingCleanupWorker`, `OvertimeWarningWorker`), giải phóng API Controller khỏi các tác vụ nền phi đồng bộ.
...
* **Dependency Inversion Principle (DIP):**
  * Áp dụng tốt thông qua việc Controllers và Services giao tiếp với nhau bằng Interfaces thay vì Concrete Classes. Hosted services chạy nền chỉ gọi tới các Interface nghiệp vụ ở tầng Application (`IBookingService`, `IParkingSessionService`).
...
* **Hạn chế:** API Controller thiếu phân quyền (Authorization) đồng nhất. Một số endpoint nhạy cảm (như thêm/xóa chính sách giá, cấu hình giá phạt, cấu hình giá tháng) chưa được gán thẻ kiểm tra Role hợp lệ (`[Authorize(Roles = "Admin,Manager")]`).
...
* **Tiến độ vượt bậc:** Dự án đã mở rộng đáng kể số lượng unit test tự động trong project `PBMS.UnitTests` (tổng số test đã đạt mốc **135 tests**, duy trì tỷ lệ pass **100%**).
* Các module cốt lõi như `BookingService`, `ParkingSessionService`, `VehicleTypeService`, `IncidentService` và `PaymentService` đã được bao phủ bởi các unit test kiểm thử các luật nghiệp vụ khắt khe. Một unit test chuyên biệt đã được bổ sung thành công cho `SendOvertimeWarningsAsync` để kiểm tra độ tin cậy của việc phát hiện xe đỗ cận giờ và chèn thông báo tự động.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| Tách biệt cấu hình DI khởi động của lớp API và Infrastructure | API / Infrastructure | Tăng tính cô lập dự án | **P1** | [Program.cs](file:///D:/FPT/SWP391/parking-system-api/src/PBMS.API/Program.cs#L43) tham chiếu trực tiếp lớp hiện thực của Infrastructure. | **Remaining** |
